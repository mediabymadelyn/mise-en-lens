...
function buildQuizPrompt(
  ...
  return [
    ...
    "- Q1 focuses on Film 1, Q2 focuses on Film 2 — do not use the same film for both warm-up questions.",
    ...
    "- Q3 and Q4 MUST focus on different films. Q3 focuses on one film, Q4 focuses on a different film from the Top 4.",
    ...
    "- Both Q5 and Q6 compare films on the SAME concept or lens — do not switch lenses between questions.",
    "- Q5 compares Film 1 and Film 2 from the Top 4. Q6 compares Film 3 and Film 4. This ensures all four films are covered.",
    ...
    "Q7 (multiple_choice, id='q7'): APPLY VERIFY — 'Which of these best describes a moment where [concept] is doing real work in [Film A]?' All four options must sound like plausible film scenes — no obviously wrong distractors. The correct option is the one where the concept actively shapes a character's choice or feeling. The three wrong options should each describe real-sounding scenes where the concept is absent, passive, or irrelevant.",
    "Q8 (short_answer, id='q8'): EVALUATE — 'Think about [Film B]. What would be lost if [concept] were removed from the film? Name one specific moment that shows why it matters.' Film B must differ from Film A. acceptableAnswers should include reasoning words: 'without', 'lose', 'lost', 'missing', 'meaning', 'different', 'empty'. acceptableKeywords should include: 'without', 'lose', 'missing', 'meaning', 'instead', 'because', 'matter', 'remove'. maxWords: 18.",
    "",
    "SECTION 5 — Reflection (Q9): short_answer",
    "- ONE question. Personal, lightweight, approachable.",
    "- The question MUST name at least one specific film from the Top 4 by title. Do NOT use 'the film' without a film name.",
    "- Example stems: 'Which moment in [Film Title] affected you the most, and how did it create that impact?', 'Has your interpretation of [Film Title] changed during this quiz? Why?'",
    "- maxWords: 25.",
    "- focus: 'Reflection'",
...
async function generateWithOpenAI<T>(
  ...
  try {
    ...
    throw new Error(isTimeout ? "OpenAI request timed out after 120s" : `OpenAI fetch failed: ${err}`);
  } finally {
    clearTimeout(timeoutId);
  }

  if (!response.ok) {
    throw new Error(`OpenAI request failed with status ${response.status}. Please try again in a moment.`);
  }
