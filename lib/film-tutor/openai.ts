...
function buildQuizPrompt(
  ...
  return [
    ...
    "Q5 (multiple_choice, id='q5'): Transfer VERIFY — 'Which of these in [Film A] is an example of [concept]?' Four options: three plausible distractors and one correct answer that is clearly supported by the teachStatement. IMPORTANT: do NOT copy the exact phrasing of the teachStatement as the correct option — paraphrase it so the student has to think, not just pattern-match. All four options should be plausible scene descriptions so the correct one is not immediately obvious.",
    ...
    "Hints must be specific and directional — name at least one concrete example the student can use. Never write generic phrases like 'Consider the film\\'s emotional journey' or 'Think about the story.' Instead write something like: 'Try one of these: identity, family, power, or loss — then say why the film keeps returning to it.'",
    "Feedback behavior: partial -> name the specific part that was right (e.g. 'You named the theme — now add one sentence about why the film keeps returning to it'), then ask ONE targeted refinement. Never write filler like 'That resonates deeply' or 'Great insight'. Correct -> one sentence confirming what was right about their specific answer.",
